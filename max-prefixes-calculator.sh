...
function printHelp {
  ...
  echo "Interfaces: how many interfaces in use within the subnet"
...
function availablePrefixes {
    ...
    do
        ...
        #  if [[ ${#PREFIXES_IN_USE[@]} -eq 0 ]] || [[ ! ${PREFIXES_IN_USE[@]} =~ $POSSIBLE_PREFIX ]]
        then
        ...
        # echo 'herreeee'
            ((LOOP_END=$CIDR_LAST_DIGIT+$IPS_PER_PREFIX-1))
              ...
              do
                  ...
                  # if [[ ${#IPS_IN_USE[@]} -gt 0 && ${IPS_IN_USE[@]} =~ ${FULL_IP} ]]
                  # then
                  #   IS_AVAILABLE=false
                  #   ((BROKEN_PREFIXES_NUM++))
                  #   break
